          ...
          {activeTab === 'company' && (
            <div className="space-y-8 max-w-4xl">
              ...
              <div className="space-y-4 bg-slate-50 p-5 rounded-2xl border border-slate-200">
                ...
                {/* Product Watermark Settings Card */}
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className="text-sm font-bold text-white">
                        제품 공식 워터마크(Watermark) 자동 적용 & 투명도 설정
                      </h4>
                    </div>
                    <span className="text-[11px] font-bold text-emerald-300 bg-emerald-950/80 px-2.5 py-0.5 rounded-full border border-emerald-500/30">
                      신규 제품 자동 각인 & 실시간 오버레이 ON
                    </span>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {/* Watermark Preview */}
                    <div className="p-3 bg-slate-950/80 rounded-xl border border-slate-700 flex flex-col items-center justify-center min-h-[140px] relative overflow-hidden">
                      <div className="absolute inset-0 flex items-center justify-center opacity-10 pointer-events-none text-[10px] font-mono tracking-widest text-slate-400">
                        PRODUCT PHOTO WATERMARK PREVIEW
                      </div>
                      <img
                        src="/watermark_baeksong.svg"
                        alt="Baeksong ENG Watermark"
                        className="max-h-24 max-w-[200px] object-contain relative z-10 drop-shadow"
                        style={{ opacity: companyInfo.watermarkOpacity || 0.42 }}
                      />
                      <span className="text-[10px] text-slate-400 mt-2 z-10">
                        투명도: <strong className="text-emerald-400">{Math.round((companyInfo.watermarkOpacity || 0.42) * 100)}%</strong>
                      </span>
                    </div>

                    {/* Controls */}
                    <div className="p-3 bg-slate-950/80 rounded-xl border border-slate-700 space-y-3 flex flex-col justify-center">
                      <div>
                        <div className="flex items-center justify-between text-xs font-bold text-slate-200 mb-1">
                          <span>워터마크 투명도 (선명도)</span>
                          <span className="text-emerald-400 font-mono">
                            {Math.round((companyInfo.watermarkOpacity || 0.42) * 100)}%
                          </span>
                        </div>
                        <input
                          type="range"
                          ...
                          max="0.85"
                          step="0.02"
                          value={companyInfo.watermarkOpacity || 0.42}
                      ...
                      <div className="pt-2 border-t border-slate-800 flex items-center justify-between">
                        <span className="text-[11px] text-slate-300">신규 업로드 자동 워터마크</span>
                        <span className="text-[11px] text-emerald-400 font-bold bg-emerald-950 px-2 py-0.5 rounded border border-emerald-700">
                          자동 적용 활성화됨
                        </span>
                      </div>
                    </div>
                          ...
                          <div className="p-3 bg-white rounded-xl border border-slate-200 space-y-2 text-xs">
                            ...
                            {editingProductForm.imageUrl && (
                              <div className="mt-2 flex items-center gap-3">
                                <div className="w-12 h-12 rounded bg-white border border-slate-200 flex items-center justify-center p-0.5 overflow-hidden shrink-0">
                                  <img src={editingProductForm.imageUrl} alt="preview" className="w-full h-full object-contain" />
                                </div>
                                <span className="text-[11px] text-slate-500">
                                  선택된 원본 고화질 이미지가 유지됩니다.
                                </span>
                              </div>
                            )}
                          </div>
